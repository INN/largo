<?php
/**
 * Functions for things that go in the header or footer
 */
...
if ( ! function_exists( 'largo_header' ) ) {
	...
	function largo_header() {
		...
		// print the text-only version of the site title.a
		printf(
				'<%1$s class="%2$s">
...
if ( ! function_exists( 'largo_copyright_message' ) ) {
	...
	function largo_copyright_message() {
		...
		if ( ! $msg ) {
			// translators: %s is the year as generated by `date( 'Y' )`.
			$msg = __( 'Copyright %s', 'largo' );
		}
		printf( esc_html( $msg ), date( 'Y' ) );
	}
}
	...
	function inn_logo() {
		?>
			<a href="//inn.org/" id="inn-logo-container">
				<img
					id="inn-logo"
					src="<?php echo esc_url( get_template_directory_uri() . '/img/inn_light.svg' ); ?>"
					alt="<?php
						echo esc_attr(
							sprintf(
								// translators: %s is the blog name.
								__( '%s is a member of the Institute for Nonprofit News', 'largo' ),
								get_bloginfo( 'name' )
							)
						);
					?>" />
		...
		if ( is_single() ) {
			if ( have_posts() ) :
				...
				if ( get_post_meta( get_the_ID(), 'permalink', true ) ) {
					echo '<meta name="syndication-source" content="' . esc_attr( get_post_meta( get_the_ID(), 'permalink', true ) ) . '" />';
				} else {
...
if ( ! function_exists( 'largo_post_metadata' ) ) {
	...
	function largo_post_metadata( $post_id, $echo = true ) {
		$out = '<meta itemprop="description" content="' . esc_attr( wp_strip_all_tags( largo_excerpt( get_post( $post_id ), 5, false, '', false ) ) ) . '" />' . "\n";
		...
		if ( has_post_thumbnail( $post_id ) ) {
			...
			$out .= '<meta itemprop="image" content="' . esc_attr( $image[0] ) . '" />';
		}
